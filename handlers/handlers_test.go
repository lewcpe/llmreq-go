...
func setupTestDB(t *testing.T) *gorm.DB {
	...
	db.AutoMigrate(&models.KeyHistory{})
...
func TestGetMe(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if strings.Contains(r.URL.Path, "/user/info/") {
			...
			json.NewEncoder(w).Encode(services.LiteLLMUser{
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
        ...
        if r.URL.Path == "/key/list" {
             ...
             if callCount == 1 {
                 ...
                 // Sync check
                 w.Write([]byte(`{"keys": [{"key": "sk-1234...", "key_alias": "test-key"}]}`))
             }
             return
        }
...
func TestGetActiveKeys(t *testing.T) {
    ...
    server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
        if r.URL.Path == "/key/list" {
            w.WriteHeader(http.StatusOK)
             w.Write([]byte(`{"keys": [{"key": "sk-active", "key_alias": "active-key", "spend": 1.0, "user_id": "test@example.com"}]}`))
            return
        }
    }))
    defer server.Close()
...
func TestGetActiveKeysSyncAlias(t *testing.T) {
    ...
    server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
        if r.URL.Path == "/key/list" {
            w.WriteHeader(http.StatusOK)
             w.Write([]byte(`{"keys": [{"key": "sk-new-id", "key_alias": "alias-match", "user_id": "test@example.com"}]}`))
            return
        }
    }))
    defer server.Close()
...
func TestCreateKeyLimits(t *testing.T) {
    ...
    server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
        if r.URL.Path == "/key/list" {
             ...
             for i := 0; i < 10; i++ {
                 keys[i] = map[string]interface{}{"key": "k"}
             }
